...
`rethinking` and `brms` several funcitons are used that can be found in the
`eflINLA` package.
...
sim$intr <- quantile2(sim$data$height, probs = sim$probs)
# sim$intr
```
...
```{r}
b04M07ctr_post <- brms::posterior_samples(b04M07ctr)
b04M07nat_post <- brms::posterior_samples(b04M07nat)
?as_draws
```
...
```{r}
# stop("Here: remove when not used anymore")
# TEMP
# str(i04M07ctr$misc$theta.tags)
# i04M07ctr$misc$configs$contents$tag
# names(i04M07ctr$misc)
# INLA::inla.posterior.sample(n = 1, result = i04M07ctr,
#                             selection = list("Predictor" = 1))
# INLA::inla.show.hyperspec(i04M07ctr)
# i04M07ctr$nhyper
# i04M07ctr$all.hyper$random
# i04M07ctr$names.fixed
# names(i04M07ctr$marginals.hyperpar)
# str(i04M07ctr$.args$data)
```
