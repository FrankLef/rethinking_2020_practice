...
ggplot(samples$fitted, aes(x = temp, y = doy, color = year)) +
  ...
  labs(title = sprintf("Fitted Cherry Blossom in Japan with %d knots", length(knots)),
       subtitle = "4H5")
```
...
sim <- list(n = 1000L)
# the knots used
sim$knots <- quantile(data04H05$temp, probs = seq(from = 0, to = 1, length.out = 12))
# the predictors used
sim$preds <- seq_range(data04H05$temp, n = 20L)
...
# to my taste
p <- list()
p$A <- ggplot(data=data04H05,mapping=aes(x = temp, y = doy, color = year)) +
  geom_vline(xintercept = knots, color = "slateblue", alpha = 1/2) +
  ...
  theme(title = element_text(color = "midnightblue"),
        legend.position = "none") +
  labs(title = sprintf("Cherry Blossoms: Prior distribution with %d knots", length(knots)),
       subtitle = "with sd = 10")
```
...
sim$data <- sim_prior_gam(B=sim$B, n = sim$n,
                          alpha_prior = list(mean = 100, sd = 20),
...
# can be used interchangeably. stat_lineribbon gives the better visual
# to my taste
p$B <- ggplot(data=data04H05,mapping=aes(x = temp, y = doy, color = year)) +
  geom_vline(xintercept = knots, color = "slateblue", alpha = 1/2) +
  # stat_lineribbon(data=sim$data_lng, mapping=aes(x = temp, y = doy),
  #                 .width=c(0.5, 0.75, 0.95), size = 1, color = "lightcoral",
  #                 inherit.aes = FALSE) +
