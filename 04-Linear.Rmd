...
```{r}
# Pivot a dataframe from ggdist::point_interval to longer format, excluding
# grouping variables. Its uses tidybayes::gather_variables()
gather_intervals <- function(df, incl = c(".width", ".point", ".interval"),
                             fun=ggdist::median_qi, ...) {
  # This function is largely inspired by tidybayes::gather_variables()
  # for the trick on getting grouped variables.
  # NOTE: tidybayes::gather_variables uses group_by_at which is superseded by
  #       group(by(across(...)))
  ...
  groups_ = group_vars(df)
  all_groups <- incl
  if (length(groups_) != 0) all_groups <- c(groups_, all_groups)
  
  out %>%
    group_by(across(all_groups)) %>%
    tidybayes::gather_variables()
}
```

test `gather_intervals`
...
# summ <- df %>%
#   group_by(model) %>%
#   ggdist::mode_qi(.width = the_width)
# summ
...
summ <- df %>% 
  ...
  # tally()
summ
cnt <- length(unique(df$model)) * (length(df) - 1) *
  length(c("", "lower", "upper")) * length(the_width)
stopifnot(nrow(summ) == cnt)
...
summ <- df %>% 
  ...
  # tally()
summ
cnt <- (length(df) - 1) * length(c("", "lower", "upper")) * length(the_width)
stopifnot(nrow(summ) == cnt)
```
...
`rethinking` and `brms` some functions are used that can be found in the
`eflINLA` package.
...
ggplot(sim$data) +
  ...
  geom_density(data = data04M07, aes(x = height, y = ..scaled.., color = "observed"),
               size = 1, alpha = 0.8) +
  geom_vline(xintercept = c(ggdist::Mode(sim$data$height),
...
samples$all <- samples$ctr %>%
  bind_rows(samples$nat)

# plot the variables by model
ggplot(samples$all, aes(x = value, color = model)) +
  # geom_density(aes(y = ..scaled..),
  #              size = 1, alpha = 0.8) +
  stat_halfeye(aes(fill = model), .width = c(0.5, 0.75),
               color = "purple", alpha = 0.5) +
  # scale_fill_paletteer_d("ggsci::default_nejm") +
  scale_color_manual(values = c("ctr" = "turquoise4", "nat" = "violetred")) +
  facet_grid(model ~ var, scales = "free") +
  theme_ggdist() +
...
ggplot(., aes(x = value, color = model)) +
  ...
  theme_ggdist() +
  theme(legend.position = "none") +
  labs(title = "Posterior comparisons by model", x= NULL, y = NULL)
} 
```
