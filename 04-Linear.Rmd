...
gather_interval <- function(df, value_var, fun=ggdist::median_qi, 
                            width = c(2/3, 0.89),
                            names_to="stat", values_to="value") {
  val_col <- deparse1(substitute(value_var))
  df %>%
    fun({{value_var}}, .width = width) %>%
    select(-.interval) %>%
...
df <- modelr::sim2 %>%
  group_by(x) %>%
  gather_interval(value_var = y, fun = median_qi, width = c(2/3, 0.89))
...
vars <- c("(Intercept)", "sd for the Gausssian observations   ",
          "Beta for weight_c")
vars <- rename_inlavars(vars)
# str(out)
stopifnot(identical(vars, c("b_Intercept", "sigma", "b_weight_c")))
rm(vars)
```
...
sim <- list(n = 1e4L, width = c(2/3, 0.95, 1), fun = ggdist::median_qi)
set.seed = 4
sim$data <-data.frame(
  mu = rnorm(n = sim$n, mean = 0, sd = 10),
  sigma = rexp(n = sim$n, rate = 1)
  ) %>%
  mutate(height = rnorm(n = sim$n, mean = mu, sd = sigma))
# compute the confidence intervals
sim$intervals <- sim$data %>%
  # see gather_interval() at the beginning
  gather_interval(value_var = sim$data$height, fun = sim$fun, width = sim$width)
```
...
ggplot(data = sim$data, mapping = aes(x = height)) +
  ggdist::stat_halfeye(aes(fill = stat(cut_cdf_qi(cdf, .width = sim$width))),
                       color = "darkorange") +
  scale_x_continuous(breaks = sim$intervals$value, 
                     labels = round(sim$intervals$value, 1)) +
  scale_fill_paletteer_d("ggsci::indigo_material", 
                         direction = 1,
                         name = "levels %",
                         labels = round(100 * sim$width, 0),
                         na.translate = FALSE) +
  ggrepel::geom_text_repel(data = sim$intervals,
...
# the predictor's values
priors$preds <- modelr::seq_range(x = P04M07$data$weight_c, n = priors$npreds)
# the samples of priors
priors$samples <- with(priors, {
  data.frame(
    alpha = rnorm(n = nsamples, mean = alpha_mean, sd = alpha_sd),
    beta = rlnorm(n = nsamples, meanlog = beta_mean, sdlog = beta_sd),
    sigma = rexp(n = nsamples, rate = sigma_rate)
    )
  })
```
...
```{r}
set.seed(4)
# simulate the height using hypothetical weight_c
priors$sim <- priors$preds %>%
  purrr::map_dfr(.f = function(x) {
    mu <- priors$samples$alpha + x * priors$samples$beta
    y <- rnorm(n = length(mu), mean = mu, sd = priors$samples$sigma)
...
ggplot(priors$sim) +
  geom_density(aes(x = height, y = ..scaled.., color = "prior"),
               size = 1, alpha = 0.8) +
  geom_density(data = P04M07$data, aes(x = height, y = ..scaled.., color = "observed"),
               size = 1, alpha = 0.8) +
  geom_vline(xintercept = c(ggdist::Mode(priors$sim$height),
                            ggdist::Mode(P04M07$data$height)), 
...
```{r}
samples <- list(n = 1000L, width = 0.50, fun = ggdist::median_qi)

# get the samples from centered and natural scale models
samples$ctr <- extract.samples(m04M07ctr, n = samples$n)
samples$nat <- extract.samples(m04M07nat, n = samples$n)
samples$data <- samples$ctr %>%
  bind_rows(samples$nat) %>%
  mutate(model = factor(
    c(rep("center", nrow(samples$ctr)), rep("natural", nrow(samples$nat)))
    )) %>%
  pivot_longer(cols = c("a", "b", "sigma"), names_to = "vars")

# compute the intervals
samples$intervals <- samples$data %>%
  select(model, vars, value) %>%
  group_by(model, vars) %>%
  gather_interval(value_var = value, fun = samples$fun, width = samples$width)

```

the function to plot the posteriors

```{r}
# Function to create the plot of posterior with halfeye geom and
# varying fill
...
```{r}
p <- list()
p$A <- plot_post(df = filter(samples$data, vars == "a"), 
                df1 = filter(samples$intervals, vars == "a"), 
                x_var = "value", y_var = "model", fill_var = "model", 
                x1_var = "value", y1_var = "model",
                fun = samples$fun, width = samples$width, round = 0, 
                the_labs = labs(title = "Intercept", x = NULL, y = NULL)) +
  theme(legend.position = "right")
# p$A

p$B <- plot_post(df = filter(samples$data, vars == "b"), 
                df1 = filter(samples$intervals, vars == "b"), 
                x_var = "value", y_var = "model", fill_var = "model", 
                x1_var = "value", y1_var = "model",
                fun = samples$fun, width = samples$width, round = 2, 
                the_labs = labs(title = "b slope", x = NULL, y = NULL)) +
  theme(legend.position = "right")
# p$B


p$sigma <- plot_post(df = filter(samples$data, vars == "sigma"), 
                df1 = filter(samples$intervals, vars == "sigma"), 
                x_var = "value", y_var = "model", fill_var = "model", 
                x1_var = "value", y1_var = "model",
                fun = samples$fun, width = samples$width, round = 1, 
...
get_intervl <- function(df, x_pred, fun = ggdist::mean_qi, width = c(0.89)) {
  # must be exactly of length 1, do not use multiple intervals
  stopifnot(length(width) == 1)
  
  df %>%
    as.data.frame() %>%
    purrr::map_dfr(.f = fun, .width = width) %>%
    mutate(x = x_pred) %>%
    relocate(x)
}

# plot the predictions and the fit against the raw data
plot_fitpred <- function(df, df_fitted, df_predict, x_var, y_var, color_var,
                     colrs = list(
...
The constants used to extract the samples (for fitted values and predictions)

```{r}
samples <- list(
  n = 1000L,  # sample size
  width = 2/3,  # default width used
  fun = ggdist::mean_qi  # function used for intervals
  )
```
...
```{r}
samples$obj <- m04M07ctr  # the fit object
samples$weight_c <- seq_range(P04M07$data$weight_c, n = 30L)

# the posterior predictions with intervals
samples$preds <- with(samples,{
  out <- rethinking::sim(obj, data = list(weight_c = weight_c), n = n)
  get_intervl(out, x_pred = weight_c, fun = fun, width = width)
})

# the fitted values with intervals
samples$fitted <- with(samples, {
  out <- rethinking::link(obj, data = list(weight_c = weight_c), n = n)
  get_intervl(out, x_pred = weight_c, fun = fun, width = width)
})

# plot the centered model
p_ctr <- plot_fitpred(df = P04M07$data, 
                      df_fitted = samples$fitted, 
                      df_predict = samples$preds,
                      x_var = "weight_c", y_var = "height", color_var = "age",
                      the_labs = labs(title = "Model on centered scale",
                                     subtitle = sprintf("%.0f%% interval", 100 * samples$width))) +
  scale_x_continuous(breaks = scales::breaks_extended(n = 7),
                     labels = function(x) round(x + mean(P04M07$data$weight), 0))
# p_ctr
```
...
```{r}
samples$obj <- m04M07nat
samples$weight <- seq_range(P04M07$data$weight, n = 30L)

# the posterior predictions with intervals
samples$preds <- with(samples,{
  out <- rethinking::sim(obj, data = list(weight = weight), n = n)
  get_intervl(out, x_pred = weight, fun = fun, width = width)
})

# the fitted values with intervals
samples$fitted <- with(samples, {
  out <- rethinking::link(obj, data = list(weight = weight), n = n)
  get_intervl(out, x_pred = weight, fun = fun, width = width)
})


# plot natural-scale model
p_nat <- plot_fitpred(df =  P04M07$data, 
                     df_fitted = samples$fitted, df_predict = samples$preds,
...
```{r}
samples <- list(
  weight_c = seq_range(x = P04M07$data$weight_c, n = 30L),
  weight = seq_range(x = P04M07$data$weight, n = 30L),
  n = 1000L,  # sample size
  width = c(0.5, 0.75, 0.95),  # default width used
  fun = ggdist::mean_qi  # function used for intervals
  )
```
...
```{r}
p <- list()
p$df <- data.frame(weight_c = samples$weight_c) %>%
  tidybayes::add_predicted_draws(object = b04M07ctr, ndraws = samples$n)

p$ctr <- ggplot(P04M07$data, aes(x = weight_c, y = height, color = age)) +
  stat_lineribbon(p$df, mapping = aes(x = weight_c, y = .prediction), 
                  .width = samples$width, inherit.aes = FALSE,
                  show.legend = FALSE) +
  geom_point(show.legend = FALSE) +
  scale_x_continuous(breaks = scales::breaks_extended(n = 7),
                     labels = function(x) {round(x + mean(P04M07$data$weight), 1)}) +
...
# this plot is used later when comparing to inla
b04M07ctr_pred_plot <- p$ctr


p$df <- data.frame(weight = samples$weight) %>%
  tidybayes::add_predicted_draws(object = b04M07nat, ndraws = samples$n)
p$nat <- ggplot(P04M07$data, aes(x = weight, y = height, color = age)) +
  stat_lineribbon(p$df, mapping = aes(x = weight, y = .prediction), 
                  .width = samples$width, inherit.aes = FALSE,
                  show.legend = FALSE) +
  geom_point(show.legend = FALSE) +
  scale_x_continuous(breaks = scales::breaks_extended(n = 7)) +
...
p$ctr / p$nat +
  plot_annotation(title = "Comparing 2 models (same results)", subtitle = "04M07")
```
...
```{r}
i04M07hmarg <- transform_hyper_marginal(i04M07ctr)
stopifnot(length(i04M07hmarg) == 2L)
```

then create the summary

```{r}
i04M07_hyper_summ <- write_hyper_summary(margs = i04M07hmarg, 
                                          qtl = i04M07ctr$.args$quantiles)
i04M07_hyper_summ <- i04M07_hyper_summ[c(2, 1), ]  # put sd in last row
stopifnot(all.equal(dim(i04M07_hyper_summ), c(2L, 6L)))
...
```{r}
i04M07summ <- i04M07ctr$summary.fixed %>%
  bind_rows(i04M07_hyper_summ) %>%
...
```{r}
# rename the variables to compare with brm
summ <- i04M07summ %>%
...
summ <- posterior_summary(b04M07ctr) %>%
  as.data.frame() %>%
  tibble::rownames_to_column(var = "var") %>%
  filter(var != "lp__") %>%
  select(var, Estimate, Est.Error) %>%
  full_join(y = summ[, c("var", "mean", "sd")], by = "var")
summ
```
...
newdata$new <- data.frame(
  weight_c = seq_range(x = P04M07$data$weight_c, n = newdata$n),
  height = NA_real_
)
# the complete new dataframe with old and new data
newdata$data <- P04M07$data %>%
...
```{r}
# fitted <- list(
#   weight_c = seq_range(x = P04M07$data$weight_c, n = 20)
# )
# # with inla we have to redo the object with new predictor values and
# # NA for outcome
# fitted$data <- data.frame(height = NA_real_, weight_c = fitted$weight_c)
# # add original data with flag for new data
# fitted$data <- P04M07$data %>%
#   select(height, weight_c) %>%
#   bind_rows(fitted$data) %>%
#   mutate(new = c(rep(FALSE, nrow(P04M07$data)), 
#                  rep(TRUE, length(fitted$weight_c)))
#          )
```
...
i04M07ctr_args_new$data <- newdata$data
```
...
```{r}
# use the exact same arguments as the original (except data and control.predictor)
# i04M07ctr_fit <- with(i04M07ctr$.args, {
#   inla(
#     formula = formula,
#     family = family,
#     data = fitted$data,
...
fitted$inla <- i04M07ctr_new$summary.fitted.values %>%
  ...
  slice_tail(n = length(newdata$new$weight_c)) %>%
...
fitted$brm <- fitted(b04M07ctr, 
                     newdata = data.frame(weight_c = newdata$new$weight_c), 
...
p2 <- ggplot(data = fitted$brm, aes(x = weight_c, y = Estimate, ymin = Q2.5, 
                                 ymax = Q97.5)) +
...
p3 <- ggplot(data.frame(brm = fitted$brm$Estimate, 
                        inla = fitted$inla$mean), 
             aes(x = brm, y = inla)) +
  geom_point() +
  ggdist::theme_ggdist() +
...
i04M07ctr_args_new$data <- newdata$data
preds <- list(n = 1000L)
preds$inla <- do.call(INLA::inla, i04M07ctr_args_new)
...
summ$lst <- list(
  quap = as.data.frame(unclass(precis(m04M07ctr, prob=0.95)),
    ...
    tibble::rownames_to_column(var = "var"),
  brm = as.data.frame(posterior_summary(b04M07ctr, probs=c(0.025, 0.975))) %>% 
    filter(rownames(.) != "lp__") %>%
    tibble::rownames_to_column(var = "var"),
  inla = i04M07summ %>% select(-`0.5quant`,-mode, -kld)
)


summ$df <- lapply(X = seq_along(summ$lst), function(x) {
  d <- summ$lst[[x]]
  d$var <- summ$lst[["brm"]]$var
  names(d) <- names(summ$lst[["brm"]])
  summ$lst[[x]] <- d
})

# format the data to be able to create a tableau with gt
# in the next step
summ$df <- do.call(rbind, summ$df) %>%
  mutate(method = rep(names(summ$lst), each = 3)) %>%
  pivot_longer(cols = c("Estimate", "Est.Error", "Q2.5", "Q97.5"), 
               names_to = "value_id", values_to = "value") %>%
  unite(col = "value_id", value_id, method, sep = "_") %>%
  filter(grepl(pattern = "Estimate|Est[.]Error", x = value_id)) %>%
...
summ$tbl <- summ$df %>%
  ...
  tab_footnote(
    footnote = "mean",
    locations = cells_column_spanners(spanners = contains("Estimate"))
  ) %>%
  tab_footnote(
    footnote = "standard deviation",
    locations = cells_column_spanners(spanners = contains("Est.Error"))
  ) %>%
  tab_source_note(
    source_note = "Source: Rethinking, 2nd edition, Richard McElreath"
  ) %>%
  fmt_number(columns = everything(), decimals = 2) %>%
  tab_style(style = list(cell_fill(color = "mintcream")),
  ...
  tab_style(style = cell_borders(sides = c("left"),
                                 ...
                                 weight = px(1.5),
                                 style = "solid"),
            locations = list(cells_body(columns = "Est.Error_quap"),
                             cells_column_labels(columns = "Est.Error_quap"),
                             cells_column_spanners(spanners = "Est.Error"))
            ) %>%
  tab_options(
...
```{r}
nknots <- 15
knotsA <- quantile(data04M08$year, probs = seq(from = 0, to = 1, length.out = nknots))
knotsA
```
...
ggplot(data04M08, aes(x = year, y = doy, color = temp)) +
  geom_vline(xintercept = knotsA, color = "slateblue", alpha = 1/2) +
  geom_point(shape = 20, size = 2, alpha = 2/3) +
  scale_x_continuous(breaks = knotsA, labels = knotsA) +
  ...
  labs(title = sprintf("Cherry Blossom in Japan with %d knots", length(knotsA)),
       subtitle = "4M8 a)")
```
...
B <- splines::bs(x = data04M08$year, knots = knotsA[-c(1, nknots)], 
...
pA <- ggplot(df, aes(x = year, y = doy)) +
  geom_vline(xintercept = knotsA[-c(1, length(knotsA))], color = "slateblue", alpha = 1/2) +
...
```{r}
nknots <- 25
knotsB <- quantile(data04M08$year, probs = seq(from = 0, to = 1, length.out = nknots))
knotsB
```
...
library(splines)
# must specify intercept = TRUE
B <- splines::bs(x = data04M08$year, knots = knotsB[-c(1, nknots)], 
                 degree = 3, intercept = TRUE)
# str(B)
# this data.frame will be reused below with the posteriors
df_bias <- B %>%
...
pB <- ggplot(df, aes(x = year, y = doy)) +
  geom_vline(xintercept = knotsB[-c(1, length(knotsB))], color = "slateblue", alpha = 1/2) +
  geom_point(aes(color = temp)) +
  geom_lineribbon(aes(x = year, y = Estimate, ymin = Q2.5, ymax = Q97.5),
                  color = "blueviolet", fill = "cornflowerblue", alpha = 1/2) +
  scale_color_gradientn(colors = clrs) +
  ...
  theme(title = element_text(color = "midnightblue"),
        legend.position = "none",
        axis.text.x = element_text(size = rel(0.9))) +
  labs(title = sprintf("Cherry Blossom in Japan with %d knots", length(knotsB)),
       subtitle = "4M8 b)")
pB
```
...
```{r}
nknots <- 25
knotsC <- quantile(data04M08$year, probs = seq(from = 0, to = 1, length.out = nknots))
knotsC
```
...
library(splines)
# must specify intercept = TRUE
B <- splines::bs(x = data04M08$year, knots = knotsC[-c(1, nknots)], 
                 degree = 3, intercept = TRUE)
# str(B)
# this data.frame will be reused below with the posteriors
df_bias <- B %>%
...
pC <- ggplot(df, aes(x = year, y = doy)) +
  geom_vline(xintercept = knotsC[-c(1, length(knotsC))], color = "slateblue", alpha = 1/2) +
  geom_point(aes(color = temp)) +
  geom_lineribbon(aes(x = year, y = Estimate, ymin = Q2.5, ymax = Q97.5),
                  color = "blueviolet", fill = "cornflowerblue", alpha = 1/2) +
  scale_color_gradientn(colors = clrs) +
  ...
  theme(title = element_text(color = "midnightblue"),
        legend.position = "none",
        axis.text.x = element_text(size = rel(0.9))) +
  labs(title = sprintf("Cherry Blossom in Japan with %d knots and increased weight sd to 20", length(knotsC)),
       subtitle = "4M8 c)")
# pC
```
...
```{r}
summA <- data.frame(model = "A", fixef(b04M08a)) %>%
  mutate(knots = c(NA, min(data04M08$year) -1, knotsA, max(data04M08$year) + 1))
# summA
summB <- data.frame(model = "B", fixef(b04M08b)) %>%
  mutate(knots = c(NA, min(data04M08$year) -1, knotsB, max(data04M08$year) + 1))
# summB
summC <- data.frame(model = "C", fixef(b04M08c)) %>%
  mutate(knots = c(NA, min(data04M08$year) -1, knotsC, max(data04M08$year) + 1))
# summC
df <- bind_rows(summA, summB, summC) %>%
  filter(!is.na(knots)) %>%
...
See section 9.2, p. 204, of @gomez2020 on using `inla` with *B-spline*.

```{r}
library(splines)
# create newdata with dependent variable set to NA  to estimate the splines
# newdata <- list(n = 15L)
# newdata$knots <- quantile(data04M08$year, 
#                           probs = seq(from= 0 , to = 1, length.out = newdata$n))
# we use the same knots as for model  above
newdata$knots <- knotsA
...
# IMPORTANT: We must use the same parameters as for model b04M08a to 
 #           allow comparison.
i04M08 <- inla(
  formula = doy ~ 1 + bs(year, knots = newdata$knots, degree = 3, intercept = TRUE),
  control.predictor = list(compute = TRUE),
  data = newdata$data
  )
```
...
```{r}
i04M08$summary.fixed[c("1", "19"), ]
```

once we remove these extreme items we can compare `inla` and `brm`

```{r}
summ <- i04M08$summary.fixed %>%
  filter(!(rownames(i04M08$summary.fixed) %in% c("1", "19"))) %>%
  select(mean, sd, `0.025quant`, `0.975quant`)  %>%
...
summ %>%
  ...
  round(digits = 2)
```
We observe that the coefficients are very similar. The errors are very 
different however.  This aspect was not investigated in this project as
additive models are not the main scope of the book. My hypothesis is that
inla's sd shows the standard deviation of the predictions whereas brms shows
the standard deviaiton of the *mean*.
...
```{r}
ggplot(data04H05, aes(x = temp, y = doy, color = year)) +
  geom_point(size = 1.2) +
  scale_x_continuous(breaks = scales::breaks_extended(n = 7)) +
  ...
  labs(title = "cherry blossoms", subtitle = "4H5")
```
...
```{r}
knots <- quantile(data04H05$temp, probs = seq(from = 0, to = 1, length.out = 12))
```
...
ggplot(data04H05, aes(x = temp, y = doy, color = year)) +
  ...
  geom_point(shape = 20, size = 2, alpha = 2/3) +
  scale_x_continuous(breaks = knots, labels = scales::label_number(accuracy = 0.1)) +
  scale_color_paletteer_c("pals::isol") +
  theme_classic() +
  theme(title = element_text(color = "midnightblue"),
        legend.position = c(0.90, 0.85),
...
data <- list()
# must specify intercept = TRUE
data$B <- splines::bs(x = data04H05$temp, knots = knots[-c(1, length(knots))], 
                 degree = 3, intercept = TRUE)
# str(data$B)
# head(data$B)
...
data$bias <- data$B %>%
  as.data.frame() %>%
  setNames(sprintf("B%02d", seq_len(ncol(.)))) %>%
  mutate(temp = data04H05$temp) %>%
  pivot_longer(cols = -temp, names_to = "bias_func", values_to = "bias")
...
and we sample the fit

```{r}
samples <- list()
# the fitted values
# NOTE: We remove duplicated rowss to be able to plot.
#       Otherwise the ribbon does not work well in the plot
samples$fitted <- fitted(b04H05) %>%
  as.data.frame() %>%
  bind_cols(data04H05)
  # filter(temp %in% knots) %>%
...
```{r}
# NOTE: geom_lineribbon does not work well here.  geom_smooth works great.
ggplot(samples$fitted, aes(x = temp, y = doy, color = year)) +
  geom_vline(xintercept = knots, color = "slateblue", alpha = 1/2) +
  geom_point(size = 1) +
  # geom_lineribbon(samples$fitted, 
  #                 mapping = aes(x = temp, y = Estimate, ymin = Q2.5, ymax = Q97.5),
  #                 inherit.aes = FALSE,
  #                 color = "darksalmon", fill = "sandybrown", alpha = 1/2) +
  geom_smooth(mapping = aes(x = temp, y = Estimate, ymin = Q2.5, ymax = Q97.5),
...
```{r}
sim <- list(n = 50L)
posterior_summary(b04H05)
sim$coef <- posterior_summary(b04H05)[paste0("b_B", 1:14), c("Estimate", "Est.Error")]
# sim$coef
sim$intercept <- posterior_summary(b04H05)["b_Intercept", c("Estimate", "Est.Error")]
# sim$intercept
sim$sigma <- posterior_summary(b04H05)["sigma", c("Estimate", "Est.Error")]
```



```{r}
# the nb of mu's here should be equal to the nb of rows in the data
# sim$mu <-  sim$intercept["Estimate"] + data$B %*% sim$coef[, "Estimate"]
# sim$y <- rnorm(n=nrow(sim$mu), mean=sim$mu, sd=sim$sigma["Estimate"])

# this will create a matrix with one column per observation
# and the nb of rows equal to the sample size
set.seed(4)
sim$data <- sapply(X=seq_len(nrow(data$B)), FUN = function(i) {
  # get a sample of intercept
  intercept <- rnorm(n=1, 
                     mean=sim$intercept["Estimate"], 
                     sd=sim$intercept["Est.Error"])
  # get the sample of coefficients
  coefs <- sapply(X = seq_len(nrow(sim$coef)), FUN = function(j) {
    rnorm(n=1, 
          mean=sim$coef[j, "Estimate"], 
          sd=sim$coef[j, "Est.Error"])
    })
  # multiply the coefficients by the B matrix for that specific observation
  mu <- intercept + data$B[i, ] %*% coefs
  # sample the predictions
  rnorm(n=sim$n, 
        mean=mu, 
        sd=sim$sigma["Estimate"])
  })
stopifnot(all.equal(dim(sim$data), c(sim$n, nrow(data04H05))))
```
...
sim$data_lng <- sim$data %>%
  ...
  mutate(temp = rep(data04H05$temp, times = sim$n))
str(sim$data_lng)
...
ggplot(data=data04H05,mapping=aes(x = temp, y = doy, color = year)) +
  ...
  theme(title = element_text(color = "midnightblue"),
        legend.position = "none") +
  labs(title = sprintf("Predictions of cherry blossoms with %d knots", length(knots)),
       subtitle = "4H5")
```
...
```{r echo=FALSE}
message("TODO: see 4H5 above on how to do simulation of splines")
```
