...
The `INLA` has its own object and conventions. The `eflINLA` is a project
...
This practice is used to illustrate 3 different methods that can be used to
fit the model to the data.  They are from the package `rethinking` (of course),
the very well-known package `brms` that is used in @kurtz2020a and the
package `INLA` which state of the art and is explained, for example, by
@gomez2020 and @wang2018.

The package `tidybayes` is used to harmonize the different commands and 
demonstrate that the process is the same. Since there is no tidybayes command
for `INLA`, similar functions are coded in the `eflINLA` package.

The package `posterior` is used to harmonize the results from these 3 types of 
fit.
...
```{r}
postQuap <- list(ctr = list(), nat = list())

# get the posterior samples for both models (centered and natural scale)
postQuap <- within(postQuap, {
  nsamples <- 500L
  ctr$samples <- tidy_draws(m04M07ctr, n = nsamples)
  nat$samples <- tidy_draws(m04M07nat, n = nsamples)
})



# the stats in log form for the centered model
postQuap$ctr$stats <- postQuap$ctr$samples %>%
  eflStats::gather_intervals(fun = ggdist::mode_qi) %>%
  filter(!grepl(pattern = "lower$|upper$", x = .variable))

# the stats in log form for the natural model
postQuap$nat$stats <- postQuap$nat$samples %>%
  eflStats::gather_intervals(fun = ggdist::mode_qi) %>%
  filter(!grepl(pattern = "lower$|upper$", x = .variable))

# put samples together with an identifier for the model
postQuap <- within(postQuap, {
  all <- purrr::map_dfr(.x = list("ctr" = ctr$samples, "nat" = nat$samples),
                        .f = ~ data.frame(.x), .id = "model")
})

# get the stats for all model in a long form (see eflStats::gather_intervals)
postQuap <- within(postQuap, {
  stats <- purrr::map_dfr(.x = list("ctr" = ctr$stats, "nat" = nat$stats),
                        .f = ~ data.frame(.x), .id = "model")
})

# long format used for plotting
postQuap$lng <- postQuap$all %>%
  group_by(model) %>%
...
ggplot(postQuap$lng, aes(x = .value, color = model)) +
  geom_density(aes(y = ..scaled..), size = 1, alpha = 0.8) +
  geom_vline(postQuap$stats, mapping = aes(xintercept = .value, color = model),
...
postQuap$ctr$newdata <- data.frame(
  weight_c = seq_range(data04M07$weight_c, n = 30L))
postQuap$ctr$predict <- predicted_draws(m04M07ctr, 
                                        newdata = postQuap$ctr$newdata, 
                                        ndraws = postQuap$nsamples)
# get the intervals
postQuap$ctr$intr <- median_qi(postQuap$ctr$predict) %>%
...
postQuap$nat$newdata <- data.frame(
  weight = seq_range(data04M07$weight, n = 30L))
postQuap$nat$predict <- predicted_draws(m04M07nat, 
                                        newdata = postQuap$nat$newdata, 
                                        ndraws = postQuap$nsamples)
# get the intervals
postQuap$nat$intr <- median_qi(postQuap$nat$predict)

# create plot
p <- list()
p$clrs <- c("ctr" = "turquoise4", "nat" = "violetred")

p$df <- purrr::map_dfr(.x = list("ctr" = postQuap$ctr$intr, 
                                 "nat" = postQuap$nat$intr),
                       .f = ~ data.frame(.x), .id = "model")  
...
We will use `brm` only for the centered model since this is only to compare
with `quap` and `inla`.
...
We extract the samples using `tidybayes` which gives the exact same
commands as for `rethinking::quap` above.

```{r}
postBrm <- list(ctr = list(), nat = list())

# get the posterior samples for both models (centered and natural scale)
postBrm <- within(postBrm, {
  nsamples <- 500L
  ctr$samples <- tidy_draws(b04M07ctr, n = nsamples)
  nat$samples <- tidy_draws(b04M07nat, n = nsamples)
})
...
# the stats in log form for the centered model
postBrm$ctr$stats <- postBrm$ctr$samples %>%
  eflStats::gather_intervals(fun = ggdist::mode_qi) %>%
  filter(!grepl(pattern = "lower$|upper$", x = .variable)) %>%
...
```{r}
clrs <- c("ctr" = "turquoise4", "nat" = "violetred")
ggplot(postBrm$lng, aes(x = .value, color = model)) +
  geom_density(aes(y = ..scaled..), size = 1, alpha = 0.8) +
  geom_vline(postBrm$stats, mapping = aes(xintercept = .value, color = model),
             linetype = "dashed", size = 0.5) +
  scale_color_manual(values = clrs) +
  facet_wrap(. ~ .variable, scales = "free", ncol = 1) +
  theme_ggdist() +
  theme(legend.position = "right") +
  labs(title = "Posterior comparisons by model",
       subtitle = sprintf("sample size = %d", postBrm$nsamples))
```
...
```{r}
# the predictions for the centered model
postBrm$ctr$newdata <- data.frame(
  weight_c = seq_range(data04M07$weight_c, n = 30L))
postBrm$ctr$predict <- predicted_draws(m04M07ctr, 
                                        newdata = postBrm$ctr$newdata, 
                                        ndraws = postBrm$nsamples)
# get the intervals
postBrm$ctr$intr <- median_qi(postBrm$ctr$predict) %>%
  # put weight on natural scale to enable comparisons
  mutate(weight_c = weight_c + mean(data04M07$weight)) %>%
  rename(weight = weight_c)

# the predictions form the natural-scaled model
postBrm$nat$newdata <- data.frame(
  weight = seq_range(data04M07$weight, n = 30L))
postBrm$nat$predict <- predicted_draws(m04M07nat, 
                                        newdata = postBrm$nat$newdata, 
                                        ndraws = postBrm$nsamples)
# get the intervals
postBrm$nat$intr <- median_qi(postBrm$nat$predict)

# create plot
p <- list()
p$clrs <- c("ctr" = "turquoise4", "nat" = "violetred")

p$df <- purrr::map_dfr(.x = list("ctr" = postBrm$ctr$intr, 
                                 "nat" = postBrm$nat$intr),
                       .f = ~ data.frame(.x), .id = "model")  
...
$weight_c$ to demonstrate the same type of `tidybayes` functions.  The functions
follows the `tidybayes` paradigm and are identified with the suffix `inla` as
they are experimental.  They are yet to be implemented as method.
...
Marginals are also available in `INLA`.  They are an important
result of `INLA`.  They are usually very similar to the posterior.

See the example at the end of section 2.7 of chapter 2 in @gomez2020.
...
This process is automated by `eflINLA::tidy_draws_inla()` which returns
the posterior as a `draws_rvars` object from the `posterior` package that can 
be easily converted to pretty any format, e.g. by using `posterior::as_draws_df`.



```{r}
postInla <- list(nsamples = 500L)
postInla$samples <- eflINLA::tidy_marg_draws_inla(i04M07ctr,
                                                  n = postInla$nsamples)
postInla$samples %>% posterior::summarize_draws()

# and we get the point interval stats in long form
postInla$stats <- postInla$samples %>%
  as_draws_df() %>%
  eflStats::gather_intervals(fun = ggdist::mode_qi) %>%
  filter(!grepl(pattern = "lower$|upper$", x = .variable))
# postInla$stats
...
See [fitted](-#eflINLA-marg-linpred) for more details
on obtaining fitted values from `INLA`.  In this exercise, the link function
is the identity function, therefore `efliNLA::linpred_marg_draws_inla` and
`eflINLA::epred_marg_draws_inla` give the same result and can be used
interchangeably.


```{r}
postInla$fitted <- list()
# the data to fit
postInla$fitted$newdata <- data.frame(
  weight_c = seq_range(x = data04M07$weight_c, n = 30L),
  height = NA_real_)
# create an augmented inla object to be used by efliNLA::linpred_marg_draws_inla
postInla$fitted$inla_aug <- eflINLA::augment_inla(i04M07ctr, 
                                                  newdata = postInla$fitted$newdata)

# an get the marginal from inla for every predictor
postInla$fitted$data <- linpred_marg_draws_inla(postInla$fitted$inla_aug$inla, 
                                       new_pos = postInla$fitted$inla_aug$new_pos)
# str(postInla$fitted$data)
# and every predictor has its own marginal distribution
stopifnot(nvariables(postInla$fitted$data) == nrow(postInla$fitted$newdata))
```
...
p$df <- postInla$fitted$data %>%
  posterior::as_draws_df() %>%
  pivot_longer(cols = starts_with("Predictor")) %>%
  # add the original newdata as a separate column
  mutate(weight_c = rep(postInla$fitted$newdata$weight_c, 
                        times = niterations(postInla$fitted$data)))


ggplot(data04M07, aes(x = weight_c, y = height)) +
  stat_lineribbon(p$df, mapping = aes(x = weight_c, y = value),
                  color = "darkgreen", size = 1, inherit.aes = FALSE, .width = c(0.75, 0.95),
                  show.legend = FALSE) +
  ...
  theme_ggdist() +
  theme(title = element_text(color = "midnightblue")) +
  labs(title = "Fitted values with INLA", subtitle = "4M7", x = "weight")
```
...
```{r}
# create the data used for predictions
postInla$predict <- list(nsamples = 100L)
# the data to fit
postInla$predict$newdata <- data.frame(
  weight_c = seq_range(x=data04M07$weight_c, n = 30L),
  height = NA_real_)

# create an augmented inla object to be used by efliNLA::linpred_marg_draws_inla
postInla$predict$inla_aug <- eflINLA::augment_inla(i04M07ctr, 
                                                  newdata = postInla$predict$newdata)

# an get the marginal from inla for every predictor
postInla$predict$data <- predicted_draws_inla(postInla$fitted$inla_aug$inla, 
                                      new_pos = postInla$fitted$inla_aug$new_pos,
                                      n = postInla$predict$nsamples) %>%
  as_draws_df() %>%
  select(-.chain, -.iteration, -.draw)

str(postInla$predict$data)

postInla$predict$intervals <- postInla$predict$data %>%
  purrr::map_df(.f = function(x) {ggdist::mean_qi(x, .width = c(.50))}) %>%
  mutate(weight_c = postInla$predict$newdata$weight_c)
postInla$predict$intervals

```

and we plot the data. Note that that it is almost the exact same code
as for fitted values above.

```{r}
p <- list()
p$df <- postInla$predict$data %>%
  pivot_longer(cols = starts_with("Predictor")) %>%
  # add the original newdata as a separate column
  mutate(weight_c = rep(postInla$predict$newdata$weight_c, 
                        times = nrow(postInla$predict$data)))
...
ggplot(data04M07, aes(x = weight_c, y = height)) +
  ...
  theme_ggdist() +
  theme(title = element_text(color = "midnightblue")) +
  labs(title = "Predictions with INLA", subtitle = "4M7", x = "weight")
```


### Compare `quap`, `brm`, `inla` {-}

Now we compare the 3 fitting methods which is easy since all sampled data
are harmonized by ``tidybayes


```{r}
p <- list()
p$quap <- postQuap$ctr$samples %>%
  select(-c(.chain, .iteration, .draw)) %>%
  eflStats::gather_intervals_rng() %>%
  mutate(.variable = case_when(.variable == "a" ~ "b_Intercept",
                               .variable == "b" ~ "b_weight_c",
                               TRUE ~ .variable))
# p$quap
p$brm <- postBrm$ctr$samples %>%
  select(-c(.chain, .iteration, .draw)) %>%
  eflStats::gather_intervals_rng()
  # mutate(.variable = case_when(.variable == "a" ~ "b_Intercept",
  #                              .variable == "b" ~ "b_weight_c",
...
p$inla <- postInla$samples %>%
  as_draws_df() %>%
  eflStats::gather_intervals_rng()
# p$inla


p$df <- purrr::map_dfr(.x = list("quap" = p$quap, 
                                 "brm" = p$brm, 
                                 "inla" = p$inla),
                       .f = ~ data.frame(.x), .id = "model")
  # mutate(model = factor(model))

ggplot(p$df, aes(x = .value, xmin = .lower, xmax = .upper, y = model, color = model)) +
  geom_pointinterval(fatten_point = 3, size = 2) +
  geom_vline(aes(xintercept = .value, color = model), linetype = "dashed") +
  scale_x_continuous(labels = scales::label_number(accuracy = 0.1)) +
  scale_color_paletteer_d("ggthemes::calc", direction = -1) +
  ggdist::theme_ggdist() +
  theme(title = element_text(color = "midnightblue"),
        legend.position = "bottom",
        strip.text = element_text(face = "bold", size = rel(1)),
        strip.background = element_rect(fill = "lightgoldenrod")) + 
  labs(title = "Posterior mode and 95% interval with quap, brm and inla",
       subtitle = "4M7", x = NULL) +
  facet_wrap(. ~ .variable, scales = "free_x")
```

In general, inla seems to be less variable and it's sigma is much lower
than the other model.

We also compare the predictions. The results are very similar for all 3 methods.

```{r}
p <- list()

p$quap <- postQuap$ctr$predict %>%
  group_by(weight_c) %>%
  gather_intervals_rng()
# str(p$quap)


p$brm <- postBrm$ctr$predict %>%
  group_by(weight_c) %>%
  gather_intervals_rng()
# str(p$brm)


p$inla <- postInla$predict$data %>%
  pivot_longer(cols = starts_with("Predictor")) %>%
  # add the original newdata as a separate column
  mutate(weight_c = rep(postInla$predict$newdata$weight_c, 
                        times = nrow(postInla$predict$data))) %>%
...
p$df <- purrr::map_dfr(.x = list("quap" = p$quap, "brm" = p$brm, "inla" = p$inla),
                       .f = ~ data.frame(.x), .id = "model")
# glimpse(p$df)

ggplot(p$df, aes(x = weight_c, y = .value, ymin = .lower, ymax = .upper,
                 color = model)) +
  geom_pointinterval(position = position_dodge(width = 0.5),
                     fatten_point = 2, size = 1) +
  scale_x_continuous(
  ...
  theme(legend.position = "bottom",
        title = element_text(color = "midnightblue")) +
  labs(title = "Predictions mode and 95% interval with quap, brm, inla",
       subtitle = "4M7", x = "weight", y = NULL)
