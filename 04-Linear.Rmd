...
# get the stats for each model in a long form (see eflStats::gather_intervals)
postQuap$stats <- postQuap$all %>%
  group_by(model) %>%
...
# get the stats for each model in a long form (see eflStats::gather_intervals)
postBrm$stats <- postBrm$all %>%
  group_by(model) %>%
  eflStats::gather_intervals() %>%
  filter(!grepl(pattern = "lower$|upper$", x = .variable)) %>%
  filter(.variable %in% c("b_Intercept", "b_weight", "b_weight_c", "sigma")) %>%
...
postInla$fitted$newdata <- data.frame(
  weight_c = seq_range(x = data04M07$weight_c, n = 20L),
  height = NA_real_)
...
# and every predictor has its own marginal distribution
stopifnot(nvariables(postInla$predict$data) == nrow(postInla$predict$newdata))
```
...
p$df <- postInla$predict$data %>%
  ...
  mutate(weight_c = rep(postInla$predict$newdata$weight_c, 
                        times = niterations(postInla$predict$data)))
