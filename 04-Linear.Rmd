...
str(df_raw)
stopifnot(identical(dim(df_raw), c(352L, 5L)))
```
...
beta_sd <- 1
sigma_rate <- 1
n_samples <- 50  # nb of samples from the prior
n_trials <- 50  # nb of observations for each of the prior sample item
```
...
```{r}
set.seed(4)
x_pred <- modelr::seq_range(x = df_raw$weight_c ,n = n_samples)
alpha_sim <- rnorm(n = n_samples, mean = alpha_mean, sd = alpha_sd)
beta_sim <- rlnorm(n = n_samples, meanlog = beta_mean, sdlog = beta_sd)
sigma_sim <- rexp(n = n_samples, rate = sigma_rate)
mu_sim <- alpha_sim + beta_sim * x_pred

prior_pred <- purrr::map2_dfr(.x = mu_sim, .y = sigma_sim, 
                              .f = function(mu, sigma) {
                                tibble(
                                  trial = seq_len(n_trials),
                                  predict = rnorm(n = n_trials , 
                                                  mean = mu, sd = sigma)
                                )
                              }, .id = "iter") %>%
...
prac04M07$fit[[tools::file_path_sans_ext(basename(a_file))]] <- a_fit
a_fit$summary.fixed
a_fit$summary.fitted.values
a_fit$summary.linear.predictor
```
...
```{r}
a_fit$summary.random
```

```{r}
a_fit$summary.hyperpar
```



```{r}
summ <- summary(prac04M07$fit$i04M07_natural)
summ$hyperpar
```
