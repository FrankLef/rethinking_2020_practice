...
```{r}
str(a_fit$marginals.hyperpar)
str(a_fit$internal.marginals.hyperpar)
df <- data.frame(a_fit$marginals.hyperpar[[1]])
dfi <- a_fit$internal.marginals.hyperpar[[1]]
dfi <- data.frame(inla.tmarginal(marginal = dfi, fun = function(tau) 1 / sqrt(exp(tau))))

p1 <- ggplot(df, aes(x =x, y = y)) +
  geom_line() +
  labs(title = "Posterior of precision")
p1

p2 <- ggplot(dfi, aes(x = x, y = y)) +
  geom_line() +
  labs(title = "Posterior of parameter's precision")
p2
...
```{r}
a_fit$marginals.fixed$`(Intercept)`
