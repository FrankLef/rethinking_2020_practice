...
gives a different number. For example
...
transform_hyper_marginal <- function(obj, sd_name = "sd") {
  ...
  names(marg_lst) <- sub(pattern = prec_regex,
                                 replacement = sd_name,
                                 x = names(marg_lst), 
                                 ignore.case = TRUE)
  marg_lst
...
therefore the model has a constrained fixed effect which is indicated by `clinear` 
fixed effect model as described in 

Therefore we will use

```{r}
i04M07lst_ctr <- list()
i04M07lst_ctr$frmla <- formula(height ~ f(weight_c, 
                                          model = "clinear", 
                                          range = c(0, Inf)))
...
```{r}
i04M07lst_ctr$family <- "gaussian"
stopifnot(i04M07lst_ctr$family %in% names(inla.models()$likelihood))
```
...
```{r}
i04M07lst_ctr$hyper <- list(prec = list(prior = "gaussian", param = c(0, 1)))
```
...
```{r}
i04M07lst_ctr$compute <- list(config=TRUE, dic = TRUE, waic = TRUE)
```
...
```{r}
i04M07lst_ctr$quantiles <- c(0.025, 0.5, 0.975)
```
...
# i04M07nat <- inla(
#   formula = i04M07lst_ctr$frmla,
#   family = i04M07lst_ctr$family,
#   data = data04M07,
#   quantiles = i04M07lst_ctr$quantiles,
#   control.fixed = i04M07lst_ctr$fixed,
#   control.family = list(
#     hyper = i04M07lst_ctr$hyper
#     ),
#   control.compute = i04M07lst_ctr$compute)
...
```{r}
i04M07lst_ctr$marg <- transform_hyper_marginal(i04M07nat)
stopifnot(length(i04M07lst_ctr$marg) == 2L)
```
...
i04M07lst_ctr$summ <- write_hyper_summary(margs = i04M07lst_ctr$marg, 
                                          qtl = i04M07lst_ctr$quantiles)
stopifnot(identical(dim(i04M07lst_ctr$summ), c(2L, 6L)))
...
inla_summ <- i04M07nat$summary.fixed %>%
  bind_rows(i04M07lst_ctr$summ) %>%
  tibble::rownames_to_column(var = "var")
inla_summ
i04M07lst_ctr$summ <- inla_summ
```
...
```{r}
inla_summ <- i04M07lst_ctr$summ %>%
  mutate(var = edit_varnames(var)) %>%
  select("var", "mean", "sd")
inla_summ
