...
samples$all <- bind_rows(samples$ctr, samples$nat) %>%
  mutate(model = rep(c("ctr", "nat"), 
                     times = c(nrow(samples$ctr), nrow(samples$nat))))
# samples$all
...
Pretty much the same kind of command as for `quap` and `brms` above. Except
these are functions, not method, as they are experimental.

```{r}
# samples <- list(n = 500)
# samples$data <- eflINLA::tidy_draws_inla(i04M07ctr, n = samples$n)
```
...
which are about the same results as with `brm` and `quap`.


#### Marginals {-}

Marginals are also available in `INLA`.  They are actually one of the important
aspect of `INLA`.  The result are most of the time very similar to the posterior.
...
This process is automated by `eflINLA::marginal_draws_inla()` which returns
the posterior as a `draws_rvars` object from the `posterior` that can be easily 
converted to pretty any format, e.g. by using `posterior::as_draws_df`.



```{r}
margs <- eflINLA::marginal_draws_inla(i04M07ctr)
margs %>% summarize_draws()
```
...
```{r}
stop("TODO: build linpred_draws_inla()")
fitted <- list()
# the data to fit
