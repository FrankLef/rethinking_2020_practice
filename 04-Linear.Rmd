...
Please see [details](#app-inla-04M07) for more details on the arguments.


```{r}
a_file <- here::here("fits", "i04M07.rds")
# i04M07 <- readRDS(file = a_file)
i04M07 <- inla(
...
```{r}
i04M07hmarg <- transform_hyper_marginal(i04M07)
stopifnot(length(i04M07marg) == 2L)
```
...
```{r}
i04M07summ <- i04M07nat$summary.fixed %>%
...
i04M07post <- bind_cols(fixed_samples, hyper_samples) %>%
...
i04M07fitted <- inla(
  formula = i04M07$.args$formula,
  family = i04M07$.args$family,
  data = data_fit,
  quantiles = i04M07$.args$quantiles,
  control.fixed = i04M07$.args$control.fixed,
  control.family = i04M07$.args$control.family,
  control.compute = i04M07$.args$control.compute,
...
```{r}
i04M07fitted_df <- i04M07fitted$summary.fitted.values %>%
  select(mean, sd, `0.025quant`, `0.975quant`) %>%
  slice_tail(n = length(xseq)) %>%
  mutate(weight_c = xseq)
str(i04M07fitted_df)
stopifnot(all.equal(dim(i04M07fitted_df), c(length(xseq), 5L)))
```
...
```{r}
p1 <- ggplot(data = i04M07fitted_df, aes(x = weight_c, y = mean, ymin = `0.025quant`,
                                  ymax = `0.975quant`)) +
...
p3 <- ggplot(data.frame(brm = brm_fit$Estimate, inla = i04M07fitted_df$mean), 
             aes(x = brm, y = inla)) +
  geom_point() +
  ggdist::theme_ggdist() +
...
# use the exact same arguments as the original (except data and control.compute)
i04M07predict <- inla(
  formula = i04M07$.args$formula,
  family = i04M07$.args$family,
  data = data_predict,
...
i04M07predict_summ <- i04M07predict$summary.linear.predictor %>%
  select(mean, sd, `0.025quant`, `0.975quant`) %>%
  slice_tail(n = length(xseq)) %>%
  mutate(weight_c = xseq)
# str(i04M07predict)
