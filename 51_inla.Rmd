...
```{r}
# linear predictor
app_i04M07args$formula <- height ~ f(weight_c, model = "clinear", range = c(0, Inf),
...
```{r}
# likelihood family
app_i04M07args$family <- "gaussian"
stopifnot(app_i04M07args$family %in% names(inla.models()$likelihood))
```
...
```{r}
app_i04M07args$control.fixed <- list(mean.intercept = 178, prec.intercept = 1 / (20^2))
```
...
```{r}
app_i04M07args$control.family <- list(
...
```{r}
app_i04M07args$control.compute <- list(config = TRUE, dic = TRUE, waic = TRUE)
```
...
```{r}
app_i04M07args$quantiles <- c(0.025, 0.5, 0.975)
```
...
```{r}
# the data
app_i04M07args$data <- data04M07
# we use do.call to be able to use the list of parameters
app_i04M07 <- do.call(inla, args = app_i04M07args)
```
...
get the fitted data in a data.frame with only the new values and plot the results

```{r}
# get the fited data 
p <- list()
p$fitted <- app_i04M07$summary.fitted.values %>%
...
the address.  My apologies.

The method seems to work for any model which is why it is the chosen method.
...
```{r}
app_i04M07$summary.linear.predictor %>%
...
```{r}
p <- list()
p$df <- app_i04M07$marginals.linear.predictor[["Predictor.353"]] %>%
  as.data.frame()
