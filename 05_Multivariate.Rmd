...
sim <- list()
sim <- within(sim, {
  def <- defData(varname = "C", dist = "normal", formula = 0, variance = 1)
  def <- defData(def, varname = "L", dist = "normal", formula = "C", variance = 1)
  def <- defData(def, varname = "P", dist = "normal", formula = "C", variance = 1)
  data <- genData(n = 100L, dtDefs = def)
})
...
To get a summary from `inla` that is similar to `posterior_summary` from `brms`
we need to

* collect the fixed and hyperparameters summaries together
* convert the hyperparameters from inla which are log of precision to
the original representation of sd.

See the function `posterior_summary_inla` from the
package `eflINLA` fwhich does that.  The package `brinla` has a
similar functions `brinla::bir.hyperpar.summary`.

```{r}
stop("TODO")
str(i05M04$summary.fixed)
str(i05M04$summary.hyperpar)
rbind(i05M04$summary.fixed[, -6], i05M04$summary.hyperpar)
1/ sqrt(i05M04$summary.hyperpar[1,1])
1/ sqrt(i05M04$summary.hyperpar[1,2])
i05M04$summary.hyperpar[1,2]
brinla::bri.hyperpar.summary()
...
i05M04_summ %>%
  eflRethinking::gt_posterior(
    labs = list(title = "Posterior Summary INLA", subtitle = "Practice 5M4"),
    qtl = list(`0.025quant` = "2.5%", `0.975quant` = "97.5%"),
    .tab_options = list(heading.background.color = "lightcoral"))
```
...
This is very significant considering that for the population in general the
