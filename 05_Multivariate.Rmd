...
The `INLA` has its own objects and conventions. To enable comparisons with
...
sim$data <- data.frame(
  C = rnorm(n = sim$n)) %>%
  mutate(L = rnorm(n = nrow(.), mean = C),
         P = rnorm(n = nrow(.), mean = C)) %>%
  mutate(across(.fns = scale))
# str(sim$data)
```

we use `INLA` which is faster

```{r}
# univariate regression of P on C
a_file <- here::here("fits", "i05M02c.rds")
i05M02c <- readRDS(file = a_file)
# i05M02c <- inla(
#   data = sim$data,
#   formula = P ~ C,
#   family = "gaussian",
...
#   )
# saveRDS(i05M02c, file = a_file)

# univariate regression of P on L
a_file <- here::here("fits", "i05M02l.rds")
i05M02l <- readRDS(file = a_file)
# i05M02l <- inla(
#   data = sim$data,
#   formula = P ~ L,
#   family = "gaussian",
...
#   quantiles = c(0.025, 0.975)
#   )
# saveRDS(i05M02l, file = a_file)

# univariate regression of P on C and L
a_file <- here::here("fits", "i05M02.rds")
i05M02 <- readRDS(file = a_file)
# i05M02 <- inla(
#   data = sim$data,
#   formula = P ~ C + L,
#   family = "gaussian",
...
#   quantiles = c(0.025, 0.975)
#   )
# saveRDS(i05M02, file = a_file)

s <- INLA::inla.posterior.sample(n = 3, result = i05M02)
row.names(s[[1]]$latent)
...
t$.args$control.compute$config


```

and looking at the posterior distributions

```{r}
sample <- list(n = 100L)
sample$C <- INLA::inla.posterior.sample(n = sample$n, result = i05M02c)
str(sample$C)
```
