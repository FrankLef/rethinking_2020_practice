...
```{r}
gg_dag <- function(dag, clrs_status = NULL, clrs = NULL, sizes = NULL) {
  ...
  dag %>% ggdag_status(color = status, text = TRUE, 
...
```{r}
coords <- data.frame(
    name = c("Out", "Var1", "Var2", "U"),
    x = c(2, 1, 3, 2),
    y = c(1, 2, 2, 3))
clrs <- paletteer::paletteer_d("fishualize::Scarus_quoyi")
dag <- dagify(
  Out ~ Var1 + Var2,
  Var1 ~ U,
  Var2 ~ U,
...
clrs <- paletteer::paletteer_d("fishualize::Scarus_quoyi")
clrs <- c("latent"=clrs[1], "exposure"=clrs[3], "outcome"=clrs[4])
p$dag <- gg_dag(dag) +
...
dag$chain <- dagify(
  ...
  O ~ V2,
  outcome = "O") %>%
  tidy_dagitty(seed = 6, layout = "linear")
