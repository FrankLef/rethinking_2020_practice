...
ggplot(dataHurricanes, aes(x = femininity, y = deaths)) +
  ...
  scale_y_continuous(breaks = scales::breaks_width(width = 25))
```
...
p$log <- ggplot(sim$data, aes(x = x, y = lambda, group = id, color = params)) +
  ...
  labs(title = "Prior distribution of lambda",
       x = "log of feminity", y = "mean (lambda) deaths") +
...
p$nat <- ggplot(sim$data, aes(x = exp(x), y = lambda, group = id, color = params)) +
  ...
  #       axis.ticks.y = element_blank()) +
  labs(title = "Prior distribution of lambda", 
       x = "feminity", y = "mean (lambda) deaths") +
  facet_wrap(. ~ params, labeller = label_parsed)
p$nat
```
...
when we plot the WAIC penalty vs the paerot K measure from loo.
...
We use the same prior for $\beta_i$ as in the previous practice.  Since $\phi$
...
This is caused by the fact that the poisson fit assume a standard deviation
equal to its mean whereas the gamma-poisson fit allows the standard deviation
$\sigma$ to using the gamma distribution.
