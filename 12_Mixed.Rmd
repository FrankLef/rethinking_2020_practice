...
```{r}
a_file <- here::here("fits", "b12H05.rds")
b12H05 <- readRDS(file = a_file)
# message("This takes at least 10 min.")
# b12H05 <- brms::brm(data = dataTrolley,
#                    family = cumulative,
...
#                              # prior(normal(0, 0.5), class = b, coef = gender2, nlpar = a),
#                              prior(normal(0, 0.5), class = b, nlpar = b),
#                              # prior(normal(0, 0.5), class = b, coef = gender2, nlpar = b),
#                              # prior(normal(0, 0.5), class = b, coef = gender1, nlpar = c),
#                              prior(normal(0, 0.5), class = b, nlpar = c)),
#                    cores = detectCores(),
#                    seed = 12)
# # message(ÈThis takes at least 2 minutes)
# b12H05 <- add_criterion(b12H05, c("loo", "waic"))
saveRDS(object = b12H05, file = a_file)
summary(b12H05)
```

### Inference {-}

Since the response is increasing, from 1 to 7, 1 being **NOT** morally permissible
and 7 being morally permissible, and since male = 1 and female = 2 for gender,
then the fact that all slope coefficients $a_gender$, $b_gender$ and $c_gender$
are negative supports the women are more bothered by contact (and action) than men.
Said otherwise, the gender $female$ reduces the probability that the response
will be in the higher range of outcomes.
...
samples <- within(samples, {
  probs = c(0.055, 0.945)
  data <- as_draws(b12H05) %>%
    summarise_draws(mean, median, ~quantile2(., probs = probs)) %>%
    filter(grepl(pattern = ".+gender", variable)) %>%
    mutate(across(.cols = where(is.numeric), .fns = round, 4))

  p <- ggplot(data, aes(x = mean, xmin = q5.5, xmax = q94.5, y = variable)) +
    geom_pointinterval(fatten_point = 5, size = 3, color = "lightblue") +
    ggrepel::geom_text_repel(aes(label = mean), color = "lightblue") +
    geom_vline(xintercept = 0, color = "white", linetype = "dashed") +
    scale_x_continuous(breaks = scales::breaks_extended(n = 7)) +
    theme(legend.title = element_blank()) +
    labs(title = "Posterior distribution of coefficients with 89% interval",
...
$$
\begin{align*}
caught_i &\sim \mathcal{ZIPoisson}(p_i, \lambda_i) \\
logit(p_i) &= \alpha_p + \beta_p \cdot camper_i\\
log(\lambda_i) &=  \alpha_\lambda + log(hours_i) + \beta_{child} \cdot child_i + \beta_{livebait} \cdot livebait_i \\
\alpha_p &\sim \mathcal{Beta}(1, 1) \\
\beta_p &\sim \mathcal{N}(0, 0.5) \\
\alpha_\lambda &\sim \mathcal{N}(0, 1) \\
\beta_{child} &\sim \mathcal{N}(0, 0.5) \\ 
\end{align*}
$$
...
```{r}
# a_file <- here::here("fits", "b12H06.rds")
# b12H06 <- readRDS(file = a_file)
# b12H06 <- brm(data = dataFish,
...
#       cores = detectCores(),
#       seed = 12)
# saveRDS(object = b12H06, file = a_file)
```
