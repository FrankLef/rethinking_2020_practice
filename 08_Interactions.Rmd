...
data(Wines2012)
data08H05 <- Wines2012 %>%
  mutate(judge = as.character(judge),
         judge = substring(judge, regexpr(pattern = " ", text = judge) + 1), 
...
rm(Wines2012)
skimr::skim(data08H05)
```

A heatmap can help to illustrate the data.
...
pd <- data08H05 %>%
  ...
  as.matrix()
# glimpse(pd)
pal <- paletteer::paletteer_d("ggthemes::Hue_Circle")
pal <- paletteer::paletteer_d("khroma::smooth_rainbow")
superheat::superheat(X = pd, 
                     ...
                     padding = 0.5,
                     title = "Wine Coefficients: Wines vs Judges\n8H5")
```
...
```{r}
a_file <- here::here(getwd(), "fits", "b08H05.rds")
b08H05 <- readRDS(file = a_file)
# b08H05 <-
#   brm(data = data08H05,
#       family = gaussian,
...
#                 prior(exponential(1), class = sigma)),
#       iter = 2000, warmup = 1000, chains = 4, cores = detectCores(), seed = 8)
# b08H05 <- brms::add_criterion(b08H05, criterion = c("waic", "loo"))
# saveRDS(b08H05, file = a_file)
# print(b08H05, digits = 3)
```


```{r}
b08H05_fitted <- fitted(b08H05) %>%
  as_tibble()
# glimpse(b08H05_fitted)
```

```{r echo=FALSE}
# get_variables(b08H05)
```

```{r}
df <- b08H05 %>%
  posterior::as_draws_df() %>%
  select(-sigma, -lp__, - .chain, -.iteration, -.draw) %>%
  pivot_longer(cols = everything(), names_to = "var") %>%
  group_by(var) %>%
  ggdist::mean_qi(.width = 0.95) %>%
  mutate(is_wine = grepl(pattern = "^b_w_", x = var),
         id = gsub(pattern = "b_w_|b_j_|judge", replacement = "", x = var)) %>%
  arrange(value)
# str(df)

ggplot(data = df, aes(y = reorder(id, value), x = value, xmin =.lower, xmax = .upper, color = is_wine)) +
  geom_pointinterval(size = 1) +
  ggrepel::geom_text_repel(aes(label = round(value, 2)), size = 3) +
  # ggrepel::geom_text_repel(aes(x = .lower - 0.1, y = id, label = id), size = 3) +
  scale_color_manual(values = c("FALSE" = "royalblue4", "TRUE" = "violetred")) + 
  ggthemes::theme_hc() +
  theme(legend.position = "none",
        title = element_text(color = "midnightblue")) +
  labs(title = "8H5: Coefficient plot",
       subtitle = "mean and qi interval of width 95%",
       x = NULL,
       y = NULL)
  # facet_wrap(. ~ is_wine)
```

Judges John Foy and Linda Murphy have the most positive ratings. Judges Robert Hogson
and Jean Cardebat have the most negative ratings


How to interpret variations: Some judges, such as JeanMMCaderbat and Robert Hodgston 
have generally more negative views whereas John Foy and Linda Murphy have 
generally more positive scores.

Notice patterns: Yes wine I2 seems to be an outlier and John Foy as well as
Linda Murphy are more positivethan all other judges.
...
```{r}
a_file <- here::here(getwd(), "fits", "b08H06.rds")
b08H06 <- readRDS(file = a_file)
# b08H06 <-
#   brm(data = data08H05,
#       family = gaussian,
...
#                 prior(exponential(1), class = sigma)),
#       iter = 2000, warmup = 1000, chains = 4, cores = detectCores(), seed = 8)
# b08H06 <- brms::add_criterion(b08H06, criterion = c("waic", "loo"))
# saveRDS(b08H06, file = a_file)
# print(b08H06, digits = 3)
```
...
df <- b08H06 %>%
  ...
  mutate(
    type = case_when(
      grepl(pattern = "^b_w_", x = var) ~ "wine",
      grepl(pattern = "^b_j_", x = var) ~ "judge",
      grepl(pattern = "^b_f_", x = var) ~ "flight",
      TRUE ~ "NA_character"),
    type = factor(type, levels = c("flight", "judge", "wine"), ordered = TRUE),
    id = gsub(pattern = "b_w_wine[.]a|b_j_judge[.]a|b_f_", replacement = "", x = var)) %>%
  arrange(value)
# str(df)
df

ggplot(data = df, aes(y = reorder(id, as.integer(type)), x = value, xmin =.lower, xmax = .upper, color = type)) +
  geom_pointinterval(size = 2, fatten_point = 4) +
  ggrepel::geom_text_repel(aes(label = round(value, 2)), size = 4) +
  geom_vline(xintercept = 0, color = "grey60") +
  scale_color_manual(values = c("flight" = "royalblue4", "judge" = "violetred", "wine" = "darkgreen")) + 
  ggthemes::theme_hc() +
  theme(legend.position = "none",
        title = element_text(color = "midnightblue")) +
  labs(title = "8H6: Coefficient plot",
       subtitle = "mean and qi interval of width 95%",
       x = NULL,
       y = NULL)
```
...
and Linda Murphy, both American judges but with clearly higher score
than all other judges.

## 8H7 {-}


See [Gregor Mathes](https://gregor-mathes.netlify.app/2021/03/22/rethinking-chapter-8/) 
for good example with `rethinking`.

We use an index instead of a dummy variable as suggested by @elreath2020 
in section 8.1.3. See the same section in @kurtz2020b on how to use an index
variable not only for the slope but the entire linear equation.

Doing the fit with `brms` as described in section 8.1.3 of @kurtz2020b.


```{r}
a_file <- here::here(getwd(), "fits", "b08H07.rds")
b08H07 <- readRDS(file = a_file)
# b08H07 <-
#   brm(data = data08H05,
#       family = gaussian,
#       formula = bf(score.s ~ 0 + f + j + w + fj + fw + jw,
#                    f ~ 0 + flight,
#                    j ~ 0 + judge.a,
#                    w ~ 0 + wine.a,
#                    fj ~ 0 + flight:judge.a,
...
#                 prior(normal(0, 0.5), nlpar = j),
#                 prior(normal(0, 0.5), nlpar = w),
#                 prior(normal(0, 0.5), nlpar = fj),
#                 prior(normal(0, 0.5), nlpar = fw),
#                 prior(normal(0, 0.5), nlpar = jw),
#                 prior(exponential(1), class = sigma)),
#       iter = 2000, warmup = 1000, chains = 4, cores = detectCores(), seed = 8)
# b08H07 <- brms::add_criterion(b08H07, criterion = c("waic", "loo"))
# saveRDS(b08H07, file = a_file)
# summary(b08H07)
```
...
We use `fitted` which is the equivalent of `rethinking::link` to get the
$mu$ values.

```{r echo=FALSE}
fitted <- list()
fitted$newdata <- expand.grid(c("red", "white"), c("AmJudge", "NotAmJudge"), 
                               c("AmWine", "NotAmWine"))
names(fitted$newdata) <- c("flight", "judge.a", "wine.a")
fitted$data <- fitted(b08H07, newdata = fitted$newdata) %>%
  as.data.frame() %>%
  bind_cols(fitted$newdata) %>%
  unite(col = "id", flight, judge.a, wine.a, sep = "-")

ggplot(data = fitted$data, aes(y = reorder(id, Estimate), 
                                x = Estimate, xmin = Q2.5, xmax = Q97.5,
                                color = Estimate)) +
  geom_pointinterval(size = 2, fatten_point = 4) +
  ggrepel::geom_text_repel(aes(label = round(Estimate, 2)), size = 4) +
  geom_vline(xintercept = 0, color = "grey60") +
  scale_color_paletteer_c("pals::isol") +
  ggthemes::theme_hc() +
  theme(legend.position = "none",
        title = element_text(color = "midnightblue")) +
  labs(title = "8H7: fitted scores by permutation of factors",
       subtitle = "mean and qi interval of width 95%",
...
**Answer to first question**: The scores seem to be different especially between American judges tasting
Non-American wines and Non-American judges tasting wines ans Non-American
judges seem to have lower scores. This is particularly true for red wines.


### Comparing models {-}

Compare model from 8H6 and 8H7

```{r}
w <- loo::loo_compare(b08H06, b08H07, criterion = "waic")
print(w, simplify = FALSE)
```

**Answer to second question**: The model from 8H7 seems less accurate. 
However the standard error of the difference is very wide, 
mode than twice the difference itself.  This might 
indicate that the comparison is not so clear cut.  Maybe because there is
not enough data for the number of parameters in 8H7 which has $6 \cdot 2 = 12$
parameters (intercept and slope by variable and interaction) and only 180
observations.
