...
ggplot(data = df, aes(y = reorder(id, value), x = value, xmin =.lower, xmax = .upper, color = is_wine)) +
  ...
  labs(title = "8H5: Coefficient plot",
       x = NULL,
       y = "mean and qi interval of width 95%")
  # facet_wrap(. ~ is_wine)
```
...
see (https://gregor-mathes.netlify.app/2021/03/22/rethinking-chapter-8/) for
good example
...
Doing the fit with `brms` as decribe in section 8.1.3 of @kurtz2020b.
...
The model from 8H7 seems less accurate. However the standard error of the
difference is very wide, mode than twice the difference itself.  This might 
