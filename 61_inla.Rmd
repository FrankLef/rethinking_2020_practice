...
impossible as the data set created with genCorData is independent of a definition
and cannot be used with additional definition with `defData` or `addColumns`, etc.

One way to do it is just below.



```{r}
sim <- list()
sim$def <- defData(varname = "x0", dist = "normal", 
                   formula = 5, variance = 1^2)
sim$def <- defData(sim$def, varname = "x1", dist = "nonrandom", 
                   formula = "..x1")
sim$def <- defData(sim$def, varname = "x2", dist = "nonrandom", 
                   formula = "..x2")
sim$def <- defData(sim$def, varname = "x3", dist = "nonrandom", 
                   formula = "..x3")
sim$def <- defData(sim$def, varname = "mu", dist = "nonrandom",
...
sim$dataCor <- genCorData(n = 10000L,
                       mu = c("x1" = 1, "x2" = 2, "x3" = 0),
...
sim$data <- injectData(dtDefs = sim$def, dtData = sim$dataCor)
# glimpse(sim$data)
```
