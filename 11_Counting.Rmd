...
sim <- within(sim, {
  defs <- defData(varname = "a", dist = "normal", formula = "..m", variance = "..s^2")
...
#                 control.fixed = list(mean.intercept = 0, prec.intercept = (1 / 2^2),
#                                      mean = 0, prec = 1 / 0.5^2),
#                 control.predictor = list(link = 1, compute = TRUE),
#                 control.compute = list(dic = TRUE, cpo = TRUE, waic = TRUE))
...
They give the same result and `inla` seems to be much less variable. Given
...
p$out <- ggplot(p$df, aes(x = paretoK, y = penaltyWAIC, color = out)) +
  geom_point() +
  geom_vline(xintercept = 0.5, linetype = "dashed", color = "violetred") +
...
Conclusion: Adding $FORESTAGE$ does not help since $FORESTAGE$ and $PCTCOVER$ 
seem to be collinear as described in section 6.1 of the textbook.  That
is, the older the forest is, the more covergae percentage we have.

Also, beyond the fact that salamnders appear when coverage is above 75%,
...
One way to improve it would be to use a different rate for each $SITE$.
...
```{r}
a_file <- here::here(getwd(), "fits", "m11H04a.rds")
m11H04a <- readRDS(a_file)
# m11H04a <- quap(
#   dataGrants,
#   flist = alist(
...
# saveRDS(m11H04a, file = a_file)
precis(m11H04a, depth = 2)
```

The model used to get the direct effect of $gender$ taking into account
the disciplines effect.

```{r}
a_file <- here::here(getwd(), "fits", "m11H04b.rds")
m11H04b <- readRDS(a_file)
m11H04b <- quap(
  dataGrants,
  flist = alist(
    awards ~ dbinom(applications, p),
    logit(p) <- bG[gender] + bD[discipline],
    bG[gender] ~ dnorm(0, 1.5),
    bD[discipline] ~ dnorm(0, 1.5)))
# saveRDS(m11H04b, file = a_file)
precis(m11H04b, depth = 2)
summary(m11H04b)
```
...
a_file <- here::here("fits", "b11H04a.rds")
b11H04a <- readRDS(a_file)
# b11H04a <- brm(data = dataGrants,
#       family = binomial,
#       formula = awards | trials(applications) ~ 0 + gender,
#       prior = prior(normal(0, 1.5), class = b),
#       cores = detectCores(), seed = 11)
# b11H04a <- brms::add_criterion(b11H04a, criterion = c("waic", "loo"))
# saveRDS(b11H04a, file = a_file)
summary(b11H04a)
```
...
```{r}
# formula = bf(awards | trials(applications) ~ a + b,
#                    a ~ 0 + gender,
#                    b ~ 0 + discipline,
#                    nl = TRUE),
#       prior= c(
...
a_file <- here::here("fits", "b11H04b.rds")
b11H04b <- readRDS(a_file)
# b11H04b <- brm(data = dataGrants,
#       family = binomial,
#       formula = awards | trials(applications) ~ 0 + gender + discipline,
#       prior= c(prior(normal(0, 1.5), class = b)),
#       cores = detectCores(), seed = 11)
# b11H04b <- brms::add_criterion(b11H04b, criterion = c("waic", "loo"))
# saveRDS(b11H04b, file = a_file)
summary(b11H04b)
```

```{r}
# a_file <- here::here("fits", "b11H04c.rds")
# b11H04c <- readRDS(a_file)
# b11H04c <- brm(data = dataGrants,
#       family = binomial,
#       formula = awards | trials(applications) ~ gender,
#       prior = c(prior(normal(1, 0.5), class = Intercept),
#                 prior(normal(0, 1.5), class = b)),
#       cores = detectCores(), seed = 11)
# b11H04c <- brms::add_criterion(b11H04c, criterion = c("waic", "loo"))
# saveRDS(b11H04c, file = a_file)
# summary(b11H04c)
```
...
```{r}
# a_file <- here::here("fits", "b11H04d.rds")
# b11H04d <- readRDS(a_file)
# b11H04d <- brm(data = dataGrants,
#       family = binomial,
#       formula = awards | trials(applications) ~ gender + discipline,
#       prior = c(prior(normal(1, 0.5), class = Intercept),
#                 prior(normal(0, 1.5), class = b)),
#       cores = detectCores(), seed = 11)
# b11H04d <- brms::add_criterion(b11H04d, criterion = c("waic", "loo"))
# saveRDS(b11H04d, file = a_file)
# summary(b11H04d)
```
...
```{r}
a_file <- here::here("fits", "i11H04a.rds")
i11H04a <- readRDS(file = a_file)
# i11H04a <- inla(data = dataGrants,
...
#                 control.predictor = list(link = 1, compute = TRUE),
#                 control.compute = list(dic = TRUE, cpo = TRUE, waic = TRUE))
# saveRDS(i11H04a, file = a_file)
summary(i11H04a)
```


```{r}
# f(-1 + gender, model = "linear")
a_file <- here::here("fits", "i11H04b.rds")
i11H04b <- readRDS(file = a_file)
# i11H04b <- inla(data = dataGrants,
#                 formula =  awards ~ -1 + gender + discipline,
#                 family = "binomial",
#                 Ntrials = applications,
#                 control.family = list(link = "logit"),
#                 control.predictor = list(link = 1, compute = TRUE),
#                 control.compute = list(dic = TRUE, cpo = TRUE, waic = TRUE))
# saveRDS(i11H04b, file = a_file)
summary(i11H04b)
i11H04b$summary.fixed
# i11H04b$summary.random
# i11H04b$summary.fitted.values
# i11H04b$summary.linear.predictor
```
...
data.frame(
  fit = c("quap", "brm", "inla"),
  waic = c(as.double(rethinking::WAIC(m11H04a)["WAIC"]), 
           brms::WAIC(b11H04a)$estimates["waic", "Estimate"],
           i11H04a$waic$waic),
  se = c(as.double(rethinking::WAIC(m11H04a)["std_err"]),
         brms::WAIC(b11H04a)$estimates["waic", "SE"],
         sd(i11H04a$waic$local.waic))) %>%
  mutate(across(where(is.double), .fns = ~round(., digits = 2))) %>%
  arrange(waic)
```
...
data.frame(
  fit = c("quap", "brm", "inla"),
  waic = c(as.double(rethinking::WAIC(m11H04b)["WAIC"]), 
           brms::WAIC(b11H04b)$estimates["waic", "Estimate"],
           i11H04b$waic$waic),
  se = c(as.double(rethinking::WAIC(m11H04b)["std_err"]),
         brms::WAIC(b11H04b)$estimates["waic", "SE"],
         sd(i11H04b$waic$local.waic))) %>%
  mutate(across(where(is.double), .fns = ~round(., digits = 2))) %>%
  arrange(waic)
```
Conclusion: `inla` seems to be a very good model with less `se` for both models.
...
# precis(m11H04b, depth = 2)
# levels(dataGrants$discipline)
# levels(dataGrants$discipline)[-1]
summ$quap <- precis(m11H04b, depth = 2) %>%
  data.frame() %>%
  tibble::rownames_to_column(var = "var") %>%
  mutate(var = c(levels(dataGrants$gender), levels(dataGrants$discipline))) %>%
  select(var, mean, sd) %>%
  mutate(fit = "quap")
summ$brm <- fixef(b11H04b) %>%
  as.data.frame() %>%
  tibble::rownames_to_column(var = "var") %>%
  mutate(var = c(levels(dataGrants$gender), levels(dataGrants$discipline))) %>%
  select(var, mean = Estimate, sd = Est.Error) %>%
  mutate(fit = "brm")
summ$inla <- i11H04b$summary.fixed %>%
  tibble::rownames_to_column(var = "var") %>%
  mutate(var = c(levels(dataGrants$gender), levels(dataGrants$discipline)[-1])) %>%
  select(var, mean, sd) %>%
  mutate(fit = "inla")
# str(summ$quap)
# str(summ$brm)
...
`m11H04b` which is ploted just below

```{r}
predict <- list()
predict <- within(predict, {
  data <- predicted_draws(b11H04a, newdata = dataGrants) %>%
    as.data.frame() %>%
    select(discipline, gender, .prediction) %>%
...
Using only gender, the predicted rates are prety much always outside of the 
observed range. 

```{r}
ggplot(predict$data %>% filter(discipline != "Human"), 
       aes(x = gender, y = prop, ymin = prop.lower, ymax = prop.upper,
                         color = gender)) +
  geom_pointinterval(shape = 1, size = 1, fatten_point = 3, position = "dodge") +
  geom_point(dataGrants %>% filter(discipline != "Human"), 
  ...
  labs(title = "Admission rate considering only gender (excluding Humanities)",
       subtitle = "Model b11H04a", x = NULL) +
  facet_wrap(. ~ discipline)
```


Using $discipline$ as well as $gender$ improves the predicition
...
predict <- list()
predict <- within(predict, {
  data <- predicted_draws(b11H04b, newdata = dataGrants) %>%
    as.data.frame() %>%
    select(discipline, gender, .prediction) %>%
    # summarize_draws(mean, ~quantile2(.,  probs = c(0.05, 0.95)))
    # select(discipline, gender, applications, awards) %>%
...
ggplot(predict$data %>% filter(discipline != "Human"), 
  ...
  scale_color_manual(values = c("f" = "purple", "m" = "navyblue")) +
  theme(legend.position = "none") +
  labs(title = "Admission rate considering gender and discipline",
...
a matter of fact, the model summary shows clearly that $gender$ is the most
influential coefficient with $gender = female$ being having a more negative
...
In addition, the sd of the discipline coefficient
is high with respect to their mean probably caused by the fact that many
different combinations of coefficients are possible with the same outcome.
This is confirmed when comparing waic.  Both model are equivalent.


```{r}
loo_compare(b11H04a, b11H04b, criterion = "waic")
```
