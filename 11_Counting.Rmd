...
Conclusion: The 2 ethods generate about the same results.  Probably caused because
the posterior is gaussian.  When the posterior is gaussian, the quadratic
approximation gives similar results to MCMC but much faster.
...
The quap offers no diagnosis and the mean is less affected by the new prior has
...
when in fact it does not reflect that actor #2 is actually a clear outlier as it always
pull the left lever as discussed in section 11.1.1 and illustrated in figure 11.4.
...
is mthe most informative as the treatment is directly related to what the actor
choose.
...
```{r}
a_file <- here::here("fits", "b11H02a_quap.rds")
# b11H02a_quap <- readRDS(file = a_file)
b11H02a_quap <- quap(
  flist = alist(
    y ~ dbinom(n, p),
    logit(p) <- a + bP[cond],
...
saveRDS(b11H02a_quap, file = a_file)
precis(b11H02a_quap, depth = 2)
```

#### `brm` fit

see b11.2 in Kurtz kurtz2020b on how to do this.

```{r}
a_file <- here::here("fits", "b11H02a_brm.rds")
b11H02a_brm <- readRDS(file = a_file)
# b11H02a_brm <- brm(data = d,
#                    family = binomial,
...
#                    )
# saveRDS(b11H02a_brm, file = a_file)
summary(b11H02a_brm)
```
...
```{r}
a_file <- here::here("fits", "b11H02a_inla.rds")
# b11H02a_inla <- readRDS(file = a_file)
b11H02a_inla <- inla(formula =  y ~ 1 + cond,
                     family = "binomial",
                     Ntrials = n,
                     data = d,
                     num.threads = 2,
                     control.compute = list(dic = TRUE, waic = TRUE))
saveRDS(b11H02a_inla, file = a_file)
summary(b11H02a_inla)
```
