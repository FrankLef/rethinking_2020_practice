...
sim <- within(sim, {
  ...
  grid <- expand.grid(mean = c(1, 2, 3), sd = c(0.5, 1, 1.5)) %>%
    mutate(model = sprintf("m=%.2f, s=%.2f", mean, sd))
...
ggplot(sim$data, aes(x = lambda, fill = model, color = model)) +
...
p$log <- ggplot(sim$data_log, aes(x = log_predictor, y = lambda, group = id, color = model)) +
  geom_line() +
...
p$hline <- 100
p$nat <- ggplot(sim$data_log, aes(x = exp(log_predictor), y = lambda, group = id, color = model)) +
  geom_line() +
...
Conclusion: Adding $FORESTAGE$ does not help as it is a spurious relation.  That
is, the older the forest is, the more covergae percentage we have.  It can be
illustrated with a plot of $PCTCOVER$ vs $FORESTAGE$.

Actually, beyond the fact that salamnders appear when coverage is above 75%,
