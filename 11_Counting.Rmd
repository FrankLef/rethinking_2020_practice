...
Conclusion: `INLA` is very fast and seems to be a better fit than `quap`.  
If we take the absolute difference between the predictions and the observed 
values as a gross indicator, then `brm` seems to be the best fit.
...
df_diff <- df %>%
  transmute(
    ...
    p_y = p_y,
    n = n,
    dp_quap = abs(p_quap - p_y),
    dp_brm = abs(p_brm - p_y),
    dp_inla = abs(p_inla - p_y),
    dmu_quap = abs(mu_quap - y),
    dmu_brm = abs(mu_brm - y),
    dmu_inla = abs(mu_inla - y),
  )
# df_diff

df_summ <- data.frame(
  fit = c("quap", "brm", "inla"),
  probs = c(sum(df_diff$dp_quap), sum(df_diff$dp_brm), sum(df_diff$dp_inla)),
  mu = c(sum(df_diff$dmu_quap), sum(df_diff$dmu_brm), sum(df_diff$dmu_inla))
)
df_summ
```
...
```{r}
# str(gt$b11H02a_predict)
# str(gt$b11H02a_epred)
...
and he predicted success count
...
reduces a less useful model.  This is caused by the fact that the variables
$A$ and $P$ are already taken into account in the variable $cond$.  The added
parameter $A*P$ increases the penalty term of the WAIC. See section 7.4.2 for
more details on waic.
